...
class CoupledLineTeeSym(QComponent):
    """Generates a four pin (+) structure comprised of a primary two pin CPW
    transmission line, two secondary pin neighboring CPW transmission
    ...
    based on coupled_line_tee and modified by Gyum Lee.
    ...
    def make(self):
        ...
        self.add_pin('second_end',
                     points=np.array(second_pin_list[2:4]),
                     width=p.second_width,
                     input_as_norm=True)
        self.add_pin('second_start',
                     points=np.array(second_pin_list[0:2][::-1]),
                     width=p.second_width,
                     input_as_norm=True)
